...
from pred_2nets_reverse.lib.utils import save_memory_banks, load_memory_banks
...
class Trainer(object):
    ...
    def train_epoch(
        ...
        for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, ids) in enumerate(
            ...
            targets_unreliable = torch.zeros(
                inputs_u.size(0), self.cfg.num_class
            ).scatter_(1, targets_unreliable.view(-1, 1), 1)

            if len(self.memory_bank_net3.memory[0]) < self.memory_bank_net3.q_length:
                consistent_indices = ids.cpu().numpy().tolist()
            else:
                consistent_indices, _ = (self.memory_bank_net3).get_consistent_samples(
                    ids.cpu().numpy(), labels_x.argmax(dim=1).cpu().numpy()
                )

            consistent_mask = torch.ones(batch_size, dtype=torch.bool)
            inconsistent_mask = ~consistent_mask
            inconsistent_inputs_x = inputs_x[inconsistent_mask]
            inconsistent_inputs_x = inconsistent_inputs_x.to(self.device)
            inconsistent_inputs_x2 = inputs_x2[inconsistent_mask]
            inconsistent_inputs_x2 = inconsistent_inputs_x2.to(self.device)
            ...
            if secondTraining:
                ...
                inputs_x = inputs_x[consistent_mask].to(self.device)
                inputs_x2 = inputs_x2[consistent_mask].to(self.device)
                labels_x = labels_x[consistent_mask].to(self.device)
                w_x = w_x[consistent_mask].to(self.device)

                if inputs_x.size(0) == 0:
                    continue  # Skip this batch if no consistent samples
                total_masked_samples += batch_size - inputs_x.size(0)

            # Check consistency for unreliable data
            consistent_unreliable_indices, csonsistent_unreliable_labels = (
                self.memory_bank_net3.get_consistent_samples(
                    index_unreliable.cpu().numpy(),
                    targets_unreliable.argmax(dim=1).cpu().numpy(),
                )
            )

            # Add consistent unreliable samples to labeled data
            consistent_unreliable_mask = torch.tensor(
                [
                    i in consistent_unreliable_indices
                    for i in index_unreliable.cpu().numpy()
                ]
            )

            consistent_unreliable_inputs_u = inputs_u[consistent_unreliable_mask].to(
                self.device
            )
            consistent_unreliable_inputs_u2 = inputs_u2[consistent_unreliable_mask].to(
                self.device
            )
            consistent_unreliable_labels_u = targets_unreliable[
                consistent_unreliable_mask
            ].to(self.device)

            inputs_x = torch.cat([inputs_x, consistent_unreliable_inputs_u], dim=0)
            inputs_x2 = torch.cat([inputs_x2, consistent_unreliable_inputs_u2], dim=0)
            labels_x = torch.cat([labels_x, consistent_unreliable_labels_u], dim=0)
            w_x = torch.cat(
                (w_x, prob_unreliable[consistent_unreliable_mask].to(self.device)),
                dim=0,
            )
            ...
            # Add inconsistent samples to the unlabeled data
            inputs_u = torch.cat([inputs_u, inconsistent_inputs_x], dim=0)
            inputs_u2 = torch.cat([inputs_u2, inconsistent_inputs_x2], dim=0)
    ...
    def train(self):
        ...
        for epoch in (
            ...
            if epoch < self.cfg.warm_up:
                ...
                self.val_train(epoch, self.net1, self.net2)
                ...
                if acc > best_acc:
                    ...
                    save_memory_banks(
                        self.cfg, self.memory_bank_net1, self.memory_bank_net2
                    )
                    torch.save(
                    ...
                    with open(
                        os.path.join(
                            self.cfg.log_dir, f"memory_bank_net1_epoch_{epoch}.pkl"
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(self.memory_bank_net1, f)
                    with open(
                        os.path.join(
                            self.cfg.log_dir, f"memory_bank_net2_epoch_{epoch}.pkl"
                        ),
                        "wb",
                    ) as f:
                        pickle.dump(self.memory_bank_net2, f)
                    ...
                    if not_improved_count >= self.cfg.early_stopping_patience:
                        self.logger.info(
                            f"Early stopping at epoch {self.early_stop_epoch} due to no improvement in validation accuracy."
                        )
                        break
    ...
    def robust_train(self):
        ...
        with open("checkpoint/safe_set.pkl", "rb") as f:
            self.memory_bank_net3 = pickle.load(f)

        self.logger.info("Memory banks loaded")
