...
from pred_ind_net.lib.utils import save_memory_banks, load_memory_banks
...
class Trainer(object):
    ...
    def train_epoch(
        ...
        for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, ids) in enumerate(
            ...
            if len(self.memory_bank_net1.memory[0]) < self.memory_bank_net1.q_length:
                consistent_indices = ids.cpu().numpy().tolist()
            else:
                consistent_indices, _ = (
                    self.memory_bank_net1 if trainNet1 else self.memory_bank_net2
                ).get_consistent_samples(
            ...
            if secondTraining:
                consistent_mask = torch.tensor(
                    [i in consistent_indices for i in ids.cpu().numpy()]
                )
                ...
                inputs_x = inputs_x[consistent_mask]
                inputs_x2 = inputs_x2[consistent_mask]
                labels_x = labels_x[consistent_mask]
                w_x = w_x[consistent_mask]

                if inputs_x.size(0) == 0:
                    continue  # Skip this batch if no consistent samples
                total_masked_samples += batch_size - inputs_x.size(0)
            ids_consistent = ids[consistent_mask]
            ...
            with torch.no_grad():
                ...
                targets_x_consistent = torch.argmax(targets_x, dim=1)

                if trainNet1:
                    self.memory_bank_net1.update_memory(
                        ids_consistent.cpu().numpy(), targets_x_consistent.cpu().numpy()
                    )
                else:
                    self.memory_bank_net2.update_memory(
                        ids_consistent.cpu().numpy(), targets_x_consistent.cpu().numpy()
                    )
    ...
    def robust_train(self):
        ...
        with open(
            "/hpc2hdd/home/mgong081/Projects/ClassificationNoisy/checkpoint/memory_bank_net1.pkl",
            "rb",
        ) as f:
            self.memory_bank_net1 = pickle.load(f)
        with open(
            "/hpc2hdd/home/mgong081/Projects/ClassificationNoisy/checkpoint/memory_bank_net2.pkl",
            "rb",
        ) as f:
            self.memory_bank_net2 = pickle.load(f)
